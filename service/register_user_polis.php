...
$Api_key=$_REQUEST['api_key'];
	...
	$cass_MstUser->select("where phone='$phone'");//cek user terdaftar
	...
	if($cekUser !=''){
		...
		$Cout3 = "0";
		$Cout4 = "Nomor anda telah terdaftar";
		$Cout = "{}";
	...
	else
	{
		...
		if($phone !='')
		{
			...
			$sms->sendSms($phone,"Anda telah terdaftar dalam E-polis Apps. User ID anda : ".$email." Password anda : ".$password);
			$mail->SendMailRegister($email,$email,$password);
			//mail($to,$subject,$txt,$headers);
			$Cout3 = $query;
			if($query=='1'){
				$Cout4 = "User ID dan Password anda akan dikirimkan melalui SMS sesaat lagi";
			}else{
